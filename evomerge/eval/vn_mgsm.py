...
class VnMGSM: 
    name = "MGSM-VN" 
    dataset_path = "TyNguyen/vn_mgsm" 
    dataset_split = "test" 
    
    def __init__(self, verbose: bool=False, strict_japanese: bool=True, ):
        self.verbose = verbose 
        self.collate_fn = None 
        dataset = load_dataset(path=self.dataset_path, split=self.dataset_split, name="default" ) 
        self.dataset = dataset.select_columns(["câu_hỏi", "đáp_án_dạng_số"]) 
    ...
    def compute_score(self, results: Dict[str, List[Any]]) -> Dict[str, float]: 
        corrects = [ answer == pred 
                     for answer, pred in zip(results["answer"], results["prediction"]) ] 
        res_dict = {"acc": mean(corrects)} # detect Japanese by fasttext and replace empty string if it's not Ja if self.lang_detect: 
        corrects_ja = [] 
        for i, resp in enumerate(results["response"]): 
            ...
            corrects_ja.append(corrects[i] and res.get("__label__vi", 0.0) > 0.5) 
            res_dict["acc_vi"] = mean(corrects_ja) 
            return res_dict 
    
    def __call__(self, model) -> EvalOutput: 
        results = [] 
        # question = [example["question"] for example in self.dataset] 
        question = [example["câu_hỏi"] for example in self.dataset] 
        # answers = [example["answer_number"] for example in self.dataset] 
        answers = [example["đáp_án_dạng_số"] for example in self.dataset] 
        # inference 
        resps = model(text=question) 
        preds = [extract_answer_number(t) for t in resps] 
        results = { "question": question, "answer": answers, "response": resps, "prediction": preds, } 
